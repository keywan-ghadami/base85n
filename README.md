...
1. Abstract
This document defines Base85N, a binary-to-text encoding scheme using a single 85-character alphabet (Alphabet-N) selected for broad compatibility and protocol friendliness. It features a 4-byte-to-5-character core conversion mechanism. An enhanced Dynamic Passthrough (DP) mode allows for more efficient and readable representation of byte sequences that are largely compatible with Alphabet-N but may contain certain predefined R-Set characters. DP mode exclusively uses Alphabet-N rules and its associated escape character (~). It uses a signaling mechanism where a 13-bit R-Set activation mask and a 9-bit length field are encoded. If an activated R-Set character (with R-Set Index j) is present in the input, it is replaced by the j-th character from a defined list of 13 "passthrough-safe" special characters (derived from Alphabet-N). The DP signal's 9-bit length field specifies the exact character count (0-511) of the following encoded output data segment. If an input data chunk suitable for DP results in an output longer than this maximum per segment, DP processing may be applied iteratively with multiple signals. A fallback to standard Base85N block encoding is used when DP is unsuitable, not more efficient, or if the original data contains bytes that cannot be represented as literals within Alphabet-N in DP mode. The scheme supports padding-free encoding and decoding.
2. Introduction
