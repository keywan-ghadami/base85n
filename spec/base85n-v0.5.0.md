...
Base85N represents arbitrary data as text, for the case where the result has to be embedded in a text-based format — JSON inside JSON, an HTML attribute, an XML text node, a configuration file, an API payload — and where the size of that result, or how cleanly it reads, matters. The input may be binary, text, or a mixture; the alphabet, not the input, is what makes the output safe to embed.

Base85N uses a single 85-character alphabet (Alphabet-N) chosen for broad protocol compatibility, so that its output requires no escaping in a JSON string, an XML text node or attribute, or a quoted HTML attribute. (Alphabet-N is not empty of every special character: it contains `` ` ``, `$` and `{`, so a JavaScript template literal and an unquoted YAML scalar are two containers that still need care.) Its core is a 4-byte-to-5-character conversion. It features two advanced compression strategies:
