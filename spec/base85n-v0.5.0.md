...
Base85N uses a single 85-character alphabet (Alphabet-N) chosen for broad protocol compatibility, so that its output requires no escaping in those containers. Its core is a 4-byte-to-5-character conversion. It features two advanced compression strategies:
